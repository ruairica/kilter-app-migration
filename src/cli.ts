...
async function main() {
	...
	if (!exportPath) {
		...
		console.error(
			"  Tip: drag your JSON file onto the exe, or onto the terminal after the command\n",
		);
		process.exitCode = 1;
		return;
	}
	...
	try {
		...
		console.error(
			`  Could not parse ${exportPath}. Check the file is valid JSON.`,
		);
		process.exitCode = 1;
		return;
	}
	...
	const circuitClimbCount =
		exportData.circuits?.reduce((s, c) => s + c.climbs.length, 0) ?? 0;

	console.log(
		`  Found: ${ascentCount} ascents, ${attemptCount} attempts, ${circuitCount} circuits (${circuitClimbCount} climbs)\n`,
	);
	...
	try {
		...
		console.error(
			`  Authentication failed: ${e instanceof Error ? e.message : e}`,
		);
		process.exitCode = 1;
		return;
	}
	...
	if (gymWalls.length === 0) {
		...
		console.log(
			`  Wall: ${selectedWall.name} (layout ${selectedWall.product_layout_uuid})\n`,
		);
	} else {
	...
	console.log(
		`    ${ascentCount} ascents, ${attemptCount} attempts, ${circuitCount} circuits`,
	);
	console.log(
		`    to ${selectedGym.name} — ${selectedWall.name} (layout ${layoutId})\n`,
	);
	...
	if (ascentCount > 0) {
		...
		process.stdout.write(
			`\r\x1b[K  Ascents: ${ascentResult.imported} imported, ${ascentResult.skipped} skipped, ${ascentResult.failed} failed\n`,
		);
	}
	...
	if (attemptCount > 0) {
		...
		console.log(
			`\r  Attempts: ${attemptResult.imported} imported, ${attemptResult.skipped} skipped, ${attemptResult.failed} failed`,
		);
	}
	...
	if (circuitCount > 0) {
		...
		console.log(
			`\r  Circuits: ${circuitResult.imported} imported, ${circuitResult.skipped} skipped, ${circuitResult.failed} failed`,
		);
	}
